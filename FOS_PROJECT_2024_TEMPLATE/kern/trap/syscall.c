...
{
   if (virtual_address == 0 || virtual_address < USER_HEAP_START || (virtual_address + size) > USER_HEAP_MAX) {
		env_exit();
	}
...
void sys_allocate_user_mem(uint32 virtual_address, uint32 size)
{
	...
	if (virtual_address == 0 || size == 0 || virtual_address < USER_HEAP_START || (virtual_address + size) > USER_HEAP_MAX) {
		env_exit();
	}
