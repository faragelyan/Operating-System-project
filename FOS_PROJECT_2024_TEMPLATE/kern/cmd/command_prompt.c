...
int s_s_c(const char *fa1, const char *fa2) {
    //subsequence
    int iter = 0;
    int ne = 0;
    int gm = 0;
    while (fa1[ne] != '\0' && fa2[gm] != '\0')
    {
        if (fa1[ne] == fa2[gm])
            ne++;
...
int process_command(int number_of_arguments, char** arguments)
{
    ...
    int real_NOArg = number_of_arguments - 1;
    LIST_INIT(&foundCommands);

    for (int j = 0; j < NUM_OF_COMMANDS; j++)
    {
        if (strcmp(arguments[0], commands[j].name) == 0)
        {
            if (real_NOArg == commands[j].num_of_args || (commands[j].num_of_args == -1 && number_of_arguments > 1))
                return j;
            else
            {
                LIST_INSERT_TAIL(&foundCommands, &commands[j]);
                return CMD_INV_NUM_ARGS;
            }
        }
    }

    // Check for subsequence matches if no exact match was found
    for (int j = 0; j < NUM_OF_COMMANDS; j++)
    {
        if (s_s_c(arguments[0], commands[j].name))
            LIST_INSERT_TAIL(&foundCommands, &commands[j]);
    }

    int n = LIST_SIZE(&foundCommands);
